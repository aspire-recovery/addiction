...
if (isset($_POST["action"])) {
    if ($_POST["action"] == 'fetch') {
        ...
        $order_column = array('appointment_table.appointment_number', 'user.u_name', 'doctor_schedule_table.doctor_schedule_date', 'appointment_table.appointment_time', 'doctor_schedule_table.doctor_schedule_day', 'appointment_table.status');

        $main_query = "SELECT * FROM appointment_table INNER JOIN doctor_schedule_table ON doctor_schedule_table.doctor_schedule_id = appointment_table.doctor_schedule_id INNER JOIN user ON user.u_id = appointment_table.patient_id ";

        $search_query = 'WHERE appointment_table.doctor_id = "' . $_SESSION["p_id"] . '" ';
        ...
        if (isset($_POST["search"]["value"])) {
            ...
            $search_query .= 'OR user.u_name LIKE "%' . $_POST["search"]["value"] . '%" ';
    ...
    if ($_POST["action"] == 'fetch_single') {
        ...
        foreach ($appointment_data as $appointment_row) {
            ...
            $object->query = "
			SELECT * FROM user 
			WHERE u_id = '" . $appointment_row["patient_id"] . "'
			";
			...
			INNER JOIN physcho 
			ON physcho.psy_id = doctor_schedule_table.doctor_id 
